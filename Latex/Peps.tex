...
\indent \\
\indent Enfin, il aura également la possibilité d'accéder à une simulation des cours futurs et de l'erreur commise à maturité pour une trajectoire particulière en supposant qu'il ai suivit aujourd'hui, et chaque fois qu'elles lui auront été proposées, les recommandations de rebalancement. Il s'agit de le convaincre du bien fondé de ces propositions.
...
Disposant alors d'un modèle, il nous faut définir les paramètres caractérisant notre produit, aux vues des propriétés de celle ci énoncé précédemment, on lui attribuera une maturité fixée par défaut à 6, un nombre de date de constatation ainsi que la taille du modèle.
